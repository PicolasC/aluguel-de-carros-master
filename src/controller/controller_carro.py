...
class Controller_Carro:
    ...
    def atualizar_carro(self) -> Carro:
        ...
        if not self.verifica_existencia_carro(chassi):
            ...
            self.mongo.db["carro"].update_one({"chassi":f"{chassi}"},{"$set": {"chassi":chassi, "cor":cor, "modelo":modelo, "marca":marca, "placa":placa, "ano":ano}})    # Recupera os dados do novo carro criado transformando em um DataFrame
            df_carro = self.recupera_carro(chassi)
            # Cria um novo objeto Carro
