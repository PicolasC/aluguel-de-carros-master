...
class Controller_Carro:
    ...
    def atualizar_carro(self) -> Carro:
        ...
        if not self.verifica_existencia_carro(chassi):
            ...
            self.relatorio.get_relatorio_categoria()
            codigo_categoria = str(input("Digite o número da nova categoria do Carro: "))
            categoria = self.valida_categoria(codigo_categoria)
            if categoria == None:
                self.mongo.close()
                print(f"A Categoria {codigo_categoria} não existe.")
            ...
            df_carro = self.recupera_carro(chassi)
            # Cria um novo objeto Carro
            carro_atualizado = Carro(df_carro.chassi.values[0],  df_carro.cor.values[0], df_carro.modelo.values[0], df_carro.marca.values[0], df_carro.placa.values[0], df_carro.ano.values[0], codigo_categoria)
            # Exibe os atributos do novo carro
    ...
    def excluir_carro(self):
        ...
        if not self.verifica_existencia_carro(chassi):        
            ...
            if opcao_excluir.lower() == "s":
                ...
                print("Atenção, os dados inteiros do carro serão excluídos! incluindo as alocações nas quais ele está presente!")
                opcao_excluir = input(f"Tem certeza que deseja excluir os dados do carro de chassi {chassi} [S ou N]: ")
                if opcao_excluir.lower() == "s":
                    # Remove o carro da tabela

                    self.mongo.db["carro"].delete_one({"chassi":f"{chassi}"})
                    self.mongo.db["alocacao"].delete_one({"chassi":f"{chassi}"})                              
